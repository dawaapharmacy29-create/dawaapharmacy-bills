...
const startOfWeek = (() => { const d = new Date(); const day = d.getDay(); const diff = day === 6 ? 0 : day + 1; d.setDate(d.getDate() - diff); return iso(d); })();
const presets = {
...
export default function SalesPurchasesReport() {
  ...
  const [custom, setCustom] = useState(presets.month);
  const range = period === "custom" ? custom : presets[period];

  const invoiceQuery = useQuery({
    queryKey: ["sales-purchases-invoices", range.from, range.to],
    queryFn: () => loadInvoices(range.from, range.to),
    staleTime: 30000,
  ...
  const deliveryQuery = useQuery({
    queryKey: ["sales-purchases-shifts", range.from, range.to],
  ...
  const isLoading = invoiceQuery.isLoading || deliveryQuery.isLoading;

  return <div dir="rtl" className="space-y-5 p-4 md:p-6">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><h1 className="text-2xl font-bold text-gray-800">المبيعات مقابل المشتريات</h1><p className="text-sm text-gray-500">يشمل كل البيانات القديمة ويُحدّث تلقائيًا مع البيانات الجديدة كل دقيقة</p></div>
      <div className="flex flex-wrap gap-2">{[["today","يومي"],["week","أسبوعي"],["month","شهري"],["custom","فترة مخصصة"]].map(([key,label]) => <Button key={key} size="sm" variant={period === key ? "default" : "outline"} onClick={() => setPeriod(key)}>{label}</Button>)}</div>
    </div>
    ...
    <Card className="p-3 text-sm text-gray-600">الفترة: <strong>{range.from}</strong> إلى <strong>{range.to}</strong></Card>
    ...
    {isLoading ? <Card className="p-8 text-center text-gray-500">جاري تحميل المقارنة...</Card> : <div className="grid gap-4 md:grid-cols-2">{visible.map((row) => <Card key={row.name} className="space-y-4 p-5">
    ...
    <Card className="p-4 text-xs text-gray-500">الحساب مباشر من البيانات الأصلية: صافي فواتير الشراء بعد المرتجعات والاستبعادات والتحويلات الداخلية، ومبيعات تسليمات الشيفت. لذلك تظهر البيانات القديمة تلقائيًا، وأي سجل جديد يدخل في التقرير دون ترحيل يدوي.</Card>
  </div>;
}
