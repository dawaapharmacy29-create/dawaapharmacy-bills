...
export default function SystemStatus() {
  ...
  const mismatch = Boolean(sync?.difference && !sync.difference.is_matched);
  const failedEvents = Number(sync?.incoming?.failed || 0);
  const healthy = !isLoading && !hasCoreError && !mismatch && failedEvents === 0;
  ...
  return <div dir="rtl" className="space-y-5 p-4 md:p-6">
    ...
    <Card className={`border p-5 ${healthy ? 'border-emerald-200 bg-emerald-50' : hasCoreError || mismatch || failedEvents > 0 ? 'border-red-200 bg-red-50' : 'border-amber-200 bg-amber-50'}`}>
      <div className="flex items-center gap-3">{healthy ? <CheckCircle2 className="h-8 w-8 text-emerald-600"/> : hasCoreError || mismatch || failedEvents > 0 ? <TriangleAlert className="h-8 w-8 text-red-600"/> : <Server className="h-8 w-8 text-amber-600"/>}<div><p className="font-bold text-slate-900">{healthy ? 'النظام والمزامنة متطابقان' : mismatch ? 'يوجد فرق بين Base44 وSupabase' : hasCoreError ? 'توجد مسارات تحتاج مراجعة' : syncQuery.isError ? 'فحص المزامنة التفصيلي غير مفعّل بعد' : 'جاري فحص النظام'}</p><p className="mt-1 text-sm text-slate-600">آخر فحص: {new Date().toLocaleString('ar-EG')}</p></div></div>
    </Card>

    {syncQuery.isError && <Card className="border-amber-200 bg-amber-50 p-4 text-sm text-amber-800"><p className="font-bold">المسارات الأساسية تعمل، لكن مقارنة المزامنة التفصيلية لم تُفعّل على قاعدة البيانات بعد.</p><p className="mt-1">يلزم تطبيق Migration: <span dir="ltr" className="font-mono">20260802052500_app_system_sync_health_readonly.sql</span>. لن يؤثر ذلك على تشغيل باقي التطبيق.</p></Card>}
    ...
    {sync && <>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="أحداث فاشلة" value={fmt(sync.incoming?.failed)} icon={TriangleAlert} danger={failedEvents > 0} />
