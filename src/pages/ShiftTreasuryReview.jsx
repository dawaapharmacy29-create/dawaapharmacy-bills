...
function Metric({ label, value, hint, className = 'text-slate-900', icon: Icon }) {
  return <div className="rounded-2xl border bg-white p-4 shadow-sm">
    <div className="flex items-start justify-between gap-2">
      <div><p className="text-[11px] text-slate-500">{label}</p><p className={`mt-1 text-xl font-bold ${className}`}>{value}</p>{hint && <p className="mt-1 text-[10px] text-slate-400">{hint}</p>}</div>
      {Icon && <div className="rounded-xl bg-slate-50 p-2"><Icon className="h-4 w-4 text-slate-500" /></div>}
    </div>
  </div>;
}
...
function QualityBadge({ shift }) {
  ...
  if (details.dataQuality === 'complete' && !hasGap(shift.sales_reconciliation_gap) && !hasGap(shift.cash_reconciliation_gap)) {
    return <span className="rounded-full bg-emerald-100 px-2 py-1 text-[11px] font-bold text-emerald-700">بيانات مكتملة</span>;
  }
  if (details.dataQuality === 'legacy') return <span className="rounded-full bg-amber-100 px-2 py-1 text-[11px] font-bold text-amber-800">تسليم قديم</span>;
...
export default function ShiftTreasuryReview() {
  ...
  async function refresh() {
    ...
    setError('');
    try {
      const data = await treasuryApi.pendingShifts({ branch, shiftType, limit: 5000 });
  ...
  const summary = useMemo(() => {
    ...
    rows.forEach((shift) => {
      ...
      if (details.dataQuality === 'complete' && !hasGap(shift.sales_reconciliation_gap) && !hasGap(shift.cash_reconciliation_gap)) complete += 1;
      else if (details.dataQuality === 'legacy') legacy += 1;
      else dataIssues += 1;
      sales += numberValue(shift.total_sales);
      expenses += numberValue(shift.total_expenses);
      net += numberValue(shift.net_amount);
    });
    return { count: rows.length, differences, complete, legacy, dataIssues, sales, expenses, net };
  }, [rows]);
  ...
  return <div dir="rtl" className="space-y-5 p-4 md:p-6">
    <header className="rounded-2xl border bg-gradient-to-l from-amber-50 via-white to-teal-50 p-4 shadow-sm md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><div className="flex items-center gap-2"><ShieldCheck className="h-6 w-6 text-amber-700"/><h1 className="text-2xl font-bold text-slate-900">مراجعة واعتماد تسليمات الشيفت</h1></div><p className="mt-1 text-sm text-slate-600">مراجعة مالية موحدة من بيانات Supabase الفعلية: المبيعات، طرق التحصيل، المصروفات، النقدية وجودة السجل قبل الترحيل.</p></div>
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4 xl:grid-cols-8">
      <Metric label="بانتظار المراجعة" value={summary.count} icon={ShieldCheck}/>
      <Metric label="بيانات مكتملة" value={summary.complete} className="text-emerald-700" icon={Database}/>
      <Metric label="تسليمات قديمة" value={summary.legacy} className={summary.legacy ? 'text-amber-700' : 'text-emerald-700'} icon={FileWarning}/>
      <Metric label="مشاكل بيانات" value={summary.dataIssues} className={summary.dataIssues ? 'text-red-600' : 'text-emerald-700'} icon={AlertTriangle}/>
      <Metric label="بها فرق نقدية" value={summary.differences} className={summary.differences ? 'text-red-600' : 'text-emerald-700'} icon={Wallet}/>
      <Metric label="إجمالي المبيعات" value={`${money(summary.sales)} ج`} className="text-blue-700" icon={CircleDollarSign}/>
      <Metric label="إجمالي المصروفات" value={`${money(summary.expenses)} ج`} className="text-red-600" icon={Landmark}/>
      <Metric label="صافي التسليمات" value={`${money(summary.net)} ج`} className="text-indigo-700" icon={Landmark}/>
    </div>

    <section className="rounded-2xl border bg-white p-4 shadow-sm">
      <div className="grid gap-3 md:grid-cols-2">
        <label className="space-y-1"><span className="text-xs font-semibold text-slate-500">الفرع</span><select value={branch} onChange={(event) => setBranch(event.target.value)} className="w-full rounded-lg border p-2 text-sm"><option value="all">كل الفروع</option><option>دواء الشامي</option><option>دواء شكري</option></select></label>
        <label className="space-y-1"><span className="text-xs font-semibold text-slate-500">الشيفت</span><select value={shiftType} onChange={(event) => setShiftType(event.target.value)} className="w-full rounded-lg border p-2 text-sm"><option value="all">كل الشيفتات</option><option>صباحي</option><option>مسائي</option><option>ليلي</option></select></label>
      </div>
    </section>
    ...
    <div className="space-y-3">
      {rows.map((shift) => {
        ...
        const hasDataIssue = details.dataQuality !== 'complete' || hasGap(shift.sales_reconciliation_gap) || hasGap(shift.cash_reconciliation_gap);
        const canApprove = (!hasDifference || differenceAcknowledged[shift.id]) && (!hasDataIssue || qualityAcknowledged[shift.id]);
        return <article id={`shift-review-${shift.id}`} key={shift.id} className={`overflow-hidden rounded-2xl border bg-white shadow-sm ${focusId === shift.id ? 'ring-2 ring-amber-400' : ''}`}>
          <button type="button" onClick={() => setExpanded(isOpen ? '' : shift.id)} className="flex w-full flex-wrap items-center justify-between gap-3 p-4 text-right">
            <div className="min-w-[280px]"><div className="flex flex-wrap items-center gap-2"><span className="font-bold text-slate-900">{shift.branch} — شيفت {shift.shift_type}</span><span className="rounded-full bg-amber-100 px-2 py-1 text-[11px] font-bold text-amber-800">انتظار اعتماد</span><QualityBadge shift={shift}/>{hasDifference && <span className="rounded-full bg-red-100 px-2 py-1 text-[11px] font-bold text-red-700">فرق {money(details.cashDifference)} ج</span>}</div><p className="mt-1 text-xs text-slate-500">{shift.shift_date} • المسؤول: {shift.submitted_by || 'غير محدد'} • آخر تحديث: {shift.updated_at ? new Date(shift.updated_at).toLocaleString('ar-EG') : '—'}</p></div>
            <div className="flex items-center gap-5"><div className="text-left"><p className="text-[11px] text-slate-400">مبيعات / مصروف / صافي</p><p className="font-bold text-blue-700">{money(shift.total_sales)} ج <span className="text-slate-300">/</span> <span className="text-red-600">{money(shift.total_expenses)} ج</span> <span className="text-slate-300">/</span> <span className="text-indigo-700">{money(shift.net_amount)} ج</span></p></div>{isOpen ? <ChevronUp className="h-5 w-5"/> : <ChevronDown className="h-5 w-5"/>}</div>
          </button>

          {isOpen && <div className="space-y-4 border-t bg-slate-50/60 p-4">
            {details.dataQuality === 'complete' ? <div className="grid grid-cols-2 gap-2 md:grid-cols-4"><Metric label="نقدي" value={`${money(details.cashSales)} ج`} className="text-emerald-700"/><Metric label="فيزا" value={`${money(details.cardSales)} ج`} className="text-blue-700"/><Metric label="تحويلات ومحافظ" value={`${money(details.transferSales)} ج`} className="text-violet-700"/><Metric label="المصروفات" value={`${money(shift.total_expenses)} ج`} className="text-red-600"/></div> : <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><strong>سجل تاريخي ناقص التفاصيل.</strong> البيانات القديمة لا تحتوي فصل طرق التحصيل والنقدية. لم يتم اختراع أو تقدير أرقام غير موجودة؛ اعتمد فقط بعد مراجعة المصدر والملاحظات.</div>}

            {details.dataQuality === 'complete' && <div className="grid grid-cols-2 gap-2 md:grid-cols-4"><Metric label="رصيد البداية" value={`${money(details.openingCash)} ج`}/><Metric label="النقدية المتوقعة" value={`${money(details.expectedCash)} ج`} className="text-indigo-700"/><Metric label="النقدية الفعلية" value={`${money(details.actualCash)} ج`} className="text-teal-700"/><Metric label="فرق الخزنة" value={`${money(details.cashDifference)} ج`} className={hasDifference ? 'text-red-600' : 'text-emerald-700'}/></div>}
            ...
            {hasDataIssue && <label className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><input type="checkbox" checked={!!qualityAcknowledged[shift.id]} onChange={(event) => setQualityAcknowledged({ ...qualityAcknowledged, [shift.id]: event.target.checked })} className="mt-1"/><span><strong>تأكيد جودة البيانات:</strong> راجعت المصدر والتفاصيل الناقصة/التاريخية، وأوافق على الاعتماد بالقيم المسجلة فقط دون أي تقدير.</span></label>}
      ...
      {!rows.length && !loading && <div className="rounded-2xl border bg-white py-16 text-center text-sm text-slate-500"><Wallet className="mx-auto mb-2 h-8 w-8 text-slate-300"/>لا توجد شيفتات معلقة مطابقة للفلاتر.</div>}
    </div>
  </div>;
}
