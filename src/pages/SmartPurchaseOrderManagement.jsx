...
import { Download, FileSpreadsheet, Save, Sparkles, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export default function SmartPurchaseOrderManagement(){
  ...
  return <div dir="rtl" className="p-4 md:p-6 space-y-5">
    <div><h1 className="text-2xl font-bold">إدارة واعتماد طلبيات المشتريات</h1><p className="text-sm text-slate-500 mt-1">عروض الموردين، تعديل الكميات والأسعار، التحسين التلقائي، الاعتماد والتصدير لكل مورد.</p></div>
    ...
    <section className="rounded-2xl border bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-4"><h2 className="font-bold flex gap-2"><FileSpreadsheet className="w-5 h-5 text-emerald-600"/>عروض الموردين</h2><button onClick={downloadOfferTemplate} className="rounded-lg border px-3 py-2 text-sm font-semibold">تنزيل قالب Excel</button></div>
      <div className="flex flex-wrap gap-3 items-center"><input type="file" accept=".xlsx,.xls,.csv" onChange={e=>e.target.files?.[0]&&readOffers(e.target.files[0])}/>{offersPreview.length>0&&<button disabled={loading} onClick={importOffers} className="rounded-lg bg-emerald-600 text-white px-4 py-2 font-semibold">استيراد {offersPreview.length} عرض</button>}</div>
    </section>

    <div className="grid lg:grid-cols-[300px_1fr] gap-4">
      <aside className="rounded-2xl border bg-white p-3 shadow-sm h-fit"><h2 className="font-bold mb-3">الطلبيات</h2><div className="space-y-2 max-h-[600px] overflow-auto">{orders.map(o=><button key={o.id} onClick={()=>openOrder(o.id)} className="w-full text-right rounded-xl border p-3 hover:bg-emerald-50"><div className="font-semibold">{o.order_number}</div><div className="text-xs text-slate-500 mt-1">{o.branch} • {o.status}</div><div className="text-sm font-bold mt-1">{money(o.approved_total)} ج</div></button>)}</div></aside>
