...
import { buildLinkedPaymentsByInvoice, getEffectiveInvoicePaid, getInvoiceRemaining, scopedGeneralPayments, validateInvoicePayment } from "@/lib/supplierFinancialIntegrity";
...
export default function SupplierBalancesBranch() {
  ...
  // Filter invoices to this branch only
  ...
  const addPayment = useMutation({
    mutationFn: async ({ invoice, amount, payment_date, notes }) => {
      const validationError = validateInvoicePayment({ invoice, amount, linkedPaymentsByInvoice });
  ...
  const saveDebt = async () => {
    setSavingDebt(true);
    const data = { supplier_name: debtDialog.supplier_name, branch, initial_debt: round2(parseFloat(debtForm.initial_debt) || 0), notes: debtForm.notes };
  ...
  const openMonthStartDialog = (supplierName) => {
    const existing = monthStarts.find(m => m.supplier_name === supplierName && m.branch === branch);
    setMonthStartForm({ month_start_date: existing?.month_start_date || "", notes: existing?.notes || "" });
    setMonthStartDialog({ supplier_name: supplierName, existing });
  };

  const saveMonthStart = async () => {
    setSavingMonthStart(true);
    const data = { supplier_name: monthStartDialog.supplier_name, branch, month_start_date: monthStartForm.month_start_date, notes: monthStartForm.notes };
    if (monthStartDialog.existing) {
      await base44.entities.SupplierMonthStart.update(monthStartDialog.existing.id, data);
    } else {
      await base44.entities.SupplierMonthStart.create(data);
    }
  ...
  const openDebtDialog = (supplierName) => {
    const existing = debts.find(d => d.supplier_name === supplierName && d.branch === branch);
  ...
  const supplierGroups = useMemo(() => {
    ...
    allSupplierNames.forEach(name => {
      ...
      const calcRemaining = (inv) => getInvoiceRemaining(inv, linkedPaymentsByInvoice);
      ...
      const generalPayments = scopedGeneralPayments(payments, name, branch);
  ...
  }, [invoices, payments, debts, allSupplierNames, monthStarts, branch, linkedPaymentsByInvoice]);
  ...
  const addDebtPayment = useMutation({
    mutationFn: async ({ supplier_name, amount, payment_date, notes }) => {
      if (!branch) throw new Error("يجب تحديد الفرع قبل تسجيل الدفعة");
  ...
  const addGeneralPayment = useMutation({
    mutationFn: async ({ supplier_name, amount, payment_date, notes }) => {
      if (!branch) throw new Error("يجب تحديد الفرع قبل تسجيل الدفعة");
      await base44.entities.SupplierPayment.create({ supplier_name, amount: round2(parseFloat(amount)), payment_date, notes: `[دفعة عامة غير موزعة] ${notes}`.trim(), branch });
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["supplier-payments"] });
  ...
  return (
    <div dir="rtl" className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className={`text-2xl font-bold ${branchColor}`}>أرصدة الموردين — {branch}</h1>
          ...
          {supplierGroups.map((group) => {
            ...
            return (
              <Card key={group.name} className="overflow-hidden">
                <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => setExpanded(isExpanded ? null : group.name)}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-xs text-gray-500">{group.invoices.length} فاتورة</p>
                  ...
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    {group.monthStartDate && (
                      <>
                        <div className="hidden sm:flex flex-col items-center bg-orange-50 border border-orange-100 rounded-lg px-3 py-1.5 min-w-[90px]">
                          <p className="text-xs text-gray-400">قديمة</p>
                        ...
                        <div className="hidden sm:flex flex-col items-center bg-blue-50 border border-blue-100 rounded-lg px-3 py-1.5 min-w-[90px]">
                          <p className="text-xs text-gray-400">جديدة</p>
                    ...
                    {isManager && (
                      <>
                        <Button size="sm" variant="outline" className="text-blue-600 border-blue-300 hover:bg-blue-50 h-7 text-xs gap-1"
                          ...
                          <Table>
                            <TableHeader>
                              <TableRow className="bg-gray-50">
                                ...
                                {isManager && <TableHead className="text-right text-xs"></TableHead>}
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                              {group.oldInvoices.map((inv) => (
                                <TableRow key={inv.id} className="hover:bg-orange-50/30">
                                  ...
                                  <TableCell className="text-green-600 text-sm">{fmt(getEffectiveInvoicePaid(inv, linkedPaymentsByInvoice))} ج</TableCell>
      ...
      <Dialog open={!!payDialog} onOpenChange={(o) => !o && setPayDialog(null)}>
        <DialogContent dir="rtl" className="max-w-sm">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button disabled={!payForm.amount || addPayment.isPending || addDebtPayment.isPending}
              onClick={() => { if (payDialog?.debtPayment) { addDebtPayment.mutate({ supplier_name: payDialog.supplier_name, ...payForm }); } else { addPayment.mutate({ invoice: payDialog.invoice, ...payForm }); } }}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <select className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                value={generalPayForm.supplier_name} onChange={e => setGeneralPayForm(f => ({ ...f, supplier_name: e.target.value }))}>
      ...
      {isManager && (
        <Dialog open={!!debtDialog} onOpenChange={(o) => !o && setDebtDialog(null)}>
          <DialogContent dir="rtl" className="max-w-sm">
            <DialogHeader><DialogTitle>المديونية القديمة — {debtDialog?.supplier_name}</DialogTitle></DialogHeader>
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 text-sm text-blue-700">
                الفرع: <strong>{branch}</strong>
              </div>
              <p className="text-sm text-gray-500">سجّل المديونية التي كانت موجودة للمورد قبل استخدام التطبيق.</p>
              <div className="space-y-1"><Label>المديونية القديمة (جنيه)</Label><Input type="number" value={debtForm.initial_debt} onChange={e => setDebtForm(f => ({ ...f, initial_debt: e.target.value }))} placeholder="0" /></div>
              <div className="space-y-1"><Label>ملاحظات</Label><Textarea value={debtForm.notes} onChange={e => setDebtForm(f => ({ ...f, notes: e.target.value }))} rows={2} /></div>
            </div>
            <DialogFooter className="gap-2">
          ...
          <DialogContent dir="rtl" className="max-w-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 rounded-lg p-3 text-sm text-blue-700">
                <p>الفواتير قبل هذا التاريخ = <strong>مديونية قديمة</strong>، والفواتير بعده = <strong>مديونية جديدة</strong>.</p>
              ...
              <div className="space-y-1">
                ...
                <Textarea value={monthStartForm.notes} onChange={e => setMonthStartForm(f => ({ ...f, notes: e.target.value }))} rows={2} />
              </div>
            </div>
            <DialogFooter className="gap-2">
              ...
              <Button disabled={!monthStartForm.month_start_date || savingMonthStart} onClick={saveMonthStart} className="bg-blue-600 hover:bg-blue-700">
                {savingMonthStart ? <Loader2 className="w-4 h-4 animate-spin" /> : "تأكيد"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
