...
const navItems = [
  ...
  { path: "/purchase-operations-review", label: "مراجعة تشغيل الطلبيات", icon: ListChecks, amber: true, adminOnly: true },
  { path: "/smart-purchase-orders", label: "رفع وتحليل ملف الطلبية", icon: BrainCircuit, indent: true },
  { path: "/smart-purchase-orders/manage", label: "تعديل وعروض الموردين", icon: FileSearch, indent: true },
  ...
  { path: "/purchase-workflow", label: "مركز دورة المشتريات", icon: GitBranch, indent: true },
  ...
  { path: "/treasury-operations", label: "رقابة وإقفال الخزنة", icon: ShieldCheck, amber: true, adminOnly: true },
...
export default function AppLayout() {
  ...
  return <div dir="rtl" className="flex min-h-screen bg-gray-50"><aside className="hidden md:flex flex-col w-60 bg-white border-l shadow-sm"><BrandHeader/><div className="px-4 py-3 border-b bg-gray-50"><p className="text-sm font-bold text-gray-800 truncate">{user?.full_name||user?.username}</p><p className="text-xs text-gray-500 truncate" dir="ltr">{user?.username}</p></div><nav className="flex-1 p-3 space-y-1 overflow-y-auto">{renderNav()}</nav><div className="p-3 border-t">{logoutButton()}</div></aside><div className="md:hidden fixed top-0 right-0 left-0 z-[60] bg-teal-600 flex items-center justify-between px-3 py-2.5 shadow-sm"><div className="flex items-center gap-2"><div className="h-9 w-9 overflow-hidden rounded-lg bg-white p-1"><img src="/dawaa-logo.jpg" alt="صيدليات دواء" className="h-full w-full object-contain"/></div><div><h1 className="text-white font-bold text-sm">صيدليات دواء</h1><p className="text-teal-100 text-[10px]">المشتريات</p></div></div><button type="button" onClick={()=>setOpen(true)} className="text-white p-2 active:bg-teal-500 rounded-lg"><Menu className="w-6 h-6"/></button></div><Sheet open={open} onOpenChange={setOpen}><SheetContent side="right" className="w-72 p-0" dir="rtl"><BrandHeader compact/><div className="px-4 py-3 border-b bg-gray-50"><p className="text-sm font-bold text-gray-800 truncate">{user?.full_name||user?.username}</p><p className="text-xs text-gray-500 truncate" dir="ltr">{user?.username}</p></div><nav className="overflow-y-auto p-3 space-y-1 h-[calc(100vh-165px)]">{renderNav(true)}</nav><div className="p-3 border-t">{logoutButton(true)}</div></SheetContent></Sheet><main className="flex-1 md:overflow-auto pt-14 md:pt-0 flex flex-col min-w-0"><div className="px-4 pt-3 pb-0 flex justify-end"><SmartAlerts/></div><div className="flex-1"><Outlet/></div></main></div>;
}