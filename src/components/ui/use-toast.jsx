...
const TOAST_VISIBLE_DURATION = 20000;
...
export const reducer = (state, action) => {
  switch (action.type) {
    ...
    case actionTypes.DISMISS_TOAST: {
      ...
      if (toastId) addToRemoveQueue(toastId); else state.toasts.forEach((item) => addToRemoveQueue(item.id));
      return { ...state, toasts: state.toasts.map((t) => t.id === toastId || toastId === undefined ? { ...t, open: false } : t) };
    }
    case actionTypes.REMOVE_TOAST:
      if (action.toastId === undefined) {
        state.toasts.forEach((item) => clearAutoDismiss(item.id));
        return { ...state, toasts: [] };
      }
...
function toast({ duration = TOAST_VISIBLE_DURATION, ...props }) {
  const id = genId();
  ...
  if (Number(duration) > 0) {
    autoDismissTimeouts.set(id, setTimeout(dismiss, Number(duration)));
  }
