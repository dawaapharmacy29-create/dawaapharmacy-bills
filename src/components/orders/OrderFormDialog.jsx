...
function genOrderNumber(){orderCounter++;return `ORD-${new Date().getFullYear()}-${String(orderCounter).slice(-4)}`;}
...
export default function OrderFormDialog({ open, onOpenChange, teamMembers = [], onSaved, editOrder = null }) {
  ...
  async function lookupCustomer(){
    ...
    try{const customer=await lookupOrderCustomerByCode(form.customer_code);setForm(p=>({...p,customer_code:customer.customer_code,customer_name:customer.customer_name||"",phone:customer.phone||"",branch:customer.branch||p.branch}));setCustomerResolved(true);}
    catch(e){setCustomerResolved(false);setError(e.message);}finally{setSearching(false);}
  }
  ...
  const handleSave=async()=>{
    setError("");
    if(!form.customer_code||!customerResolved){setError("ابحث بكود العميل الصحيح أولًا.");return;}
    if(!form.customer_name||!form.phone||!form.product_name){setError("بيانات العميل أو الصنف غير مكتملة.");return;}
    setSaving(true);
    try{
      ...
      if(editOrder)await base44.entities.CustomerOrder.update(editOrder.id,data);else await base44.entities.CustomerOrder.create(data);
      onSaved?.();onOpenChange(false);
    }catch(e){setError(e.message||"تعذر حفظ الطلب.");}finally{setSaving(false);}
  };
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-xl max-h-[92vh] overflow-y-auto px-4 py-5 sm:p-6" dir="rtl"><DialogHeader><DialogTitle className="text-teal-700">{editOrder?"تعديل الطلب":"طلب عميل جديد"}</DialogTitle></DialogHeader><div className="space-y-4">
    <section className="rounded-xl border border-teal-200 bg-teal-50/40 p-3 space-y-3"><div><label className="text-xs font-bold text-gray-700">كود العميل *</label><div className="mt-1 flex gap-2"><Input value={form.customer_code} onChange={e=>changeCode(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();lookupCustomer();}}} placeholder="اكتب كود العميل فقط" className="h-10 font-semibold" autoFocus/><Button type="button" onClick={lookupCustomer} disabled={searching||!form.customer_code.trim()} className="bg-teal-600 hover:bg-teal-700 gap-2">{searching?<Loader2 className="w-4 h-4 animate-spin"/>:<Search className="w-4 h-4"/>}بحث بالكود</Button></div><p className="mt-1 text-[11px] text-gray-500">البحث لا يعمل بالاسم أو رقم الهاتف؛ يجب إدخال كود العميل كاملًا.</p></div>
    {customerResolved&&<div className="rounded-lg border border-emerald-200 bg-white p-3"><div className="flex items-center gap-2 text-emerald-700 text-sm font-bold mb-2"><CheckCircle2 className="w-4 h-4"/>تم العثور على العميل</div><div className="grid sm:grid-cols-2 gap-2 text-sm"><div><span className="text-gray-500">الاسم: </span><b>{form.customer_name}</b></div><div><span className="text-gray-500">الهاتف: </span><b dir="ltr">{form.phone}</b></div></div></div>}
    </section>
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><div className="space-y-1"><label className="text-xs font-medium text-gray-600">اسم العميل</label><Input value={form.customer_name} readOnly placeholder="يظهر بعد البحث بالكود" className="h-9 bg-gray-50"/></div><div className="space-y-1"><label className="text-xs font-medium text-gray-600">رقم الهاتف</label><Input value={form.phone} readOnly placeholder="يظهر بعد البحث بالكود" className="h-9 bg-gray-50" dir="ltr"/></div><div className="space-y-1"><label className="text-xs font-medium text-gray-600">الفرع</label><Select value={form.branch} onValueChange={v=>set("branch",v)}><SelectTrigger className="h-9"><SelectValue placeholder="اختر الفرع"/></SelectTrigger><SelectContent>{BRANCHES.map(b=><SelectItem key={b} value={b}>{b}</SelectItem>)}</SelectContent></Select></div><div className="space-y-1"><label className="text-xs font-medium text-gray-600">مصدر الطلب</label><Select value={form.request_source} onValueChange={v=>set("request_source",v)}><SelectTrigger className="h-9"><SelectValue placeholder="المصدر"/></SelectTrigger><SelectContent>{SOURCES.map(s=><SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div></div>
    ...
    <div className="grid sm:grid-cols-2 gap-3"><div className="space-y-1"><label className="text-xs font-medium text-gray-600">تاريخ الطلب</label><Input type="date" value={form.request_date} onChange={e=>set("request_date",e.target.value)}/></div></div>
    ...
    {error&&<p className="text-sm text-red-600 text-center">{error}</p>}<div className="flex gap-2"><Button onClick={handleSave} disabled={saving||!customerResolved||!form.product_name} className="flex-1 bg-teal-600 hover:bg-teal-700">{saving?<Loader2 className="w-4 h-4 animate-spin"/>:(editOrder?"حفظ التعديلات":"حفظ الطلب")}</Button><Button variant="outline" onClick={()=>onOpenChange(false)}>إلغاء</Button></div>
