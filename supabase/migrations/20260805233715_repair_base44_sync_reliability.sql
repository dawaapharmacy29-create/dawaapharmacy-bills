...
declare
  ...
  v_role text;
begin
  select true,a.role,coalesce(a.branch_ids,array[]::text[])
  ...
  if not coalesce(v_valid,false) then return jsonb_build_object('ok',false,'error','invalid_session'); end if;
  ...
  if v_table is not null then
    if v_role='general_manager' or v_table not in ('customer_orders','expenses','pharmacy_orders','shift_deliveries','purchase_invoices') then
      ...
      if v_table='purchase_invoices' then
        execute format('select coalesce(jsonb_agg(to_jsonb(t)),''[]''::jsonb) from public.%I t where coalesce(t.is_sample,false)=false and coalesce(t.base44_sync_state,''active'')=''active'' and t.branch=any($1)',v_table) into v_rows using v_allowed_branches;
      else
        execute format('select coalesce(jsonb_agg(to_jsonb(t)),''[]''::jsonb) from public.%I t where t.branch=any($1)',v_table) into v_rows using v_allowed_branches;
      end if;
...
revoke all on function public.app_data_read_rows(text,text) from public;
grant execute on function public.app_data_read_rows(text,text) to anon,authenticated,service_role;
grant select,insert,update,delete on table public.report_settings to service_role;

create or replace function public.sync_base44_review_from_event_status()
returns trigger
language plpgsql
security definer
...
begin
  if new.status='applied' then
    update public.base44_reconciliation_reviews
       set review_status='applied',reviewed_at=coalesce(reviewed_at,now()),
           review_notes=case when review_status='applied' then review_notes else concat_ws(' ',nullif(review_notes,''),'تم تطبيق الحدث والتحقق من وجود السجل في قاعدة البيانات.') end
     where event_id=new.event_id and review_status='pending';
  elsif new.status='delete_requires_review' then
    update public.base44_reconciliation_reviews set review_status='pending',reviewed_at=null where event_id=new.event_id;
  end if;
  return new;
end;
$$;

revoke all on function public.sync_base44_review_from_event_status() from public;
drop trigger if exists trg_sync_base44_review_from_event_status on public.base44_sync_events;
create trigger trg_sync_base44_review_from_event_status
after insert or update of status on public.base44_sync_events
for each row execute function public.sync_base44_review_from_event_status();

update public.base44_reconciliation_reviews r
set review_status='applied',reviewed_at=coalesce(r.reviewed_at,e.processed_at,now()),
    review_notes=concat_ws(' ',nullif(r.review_notes,''),'تمت مطابقة حالة المراجعة مع حدث مزامنة مطبق بالفعل.')
from public.base44_sync_events e
where e.event_id=r.event_id and e.status='applied' and r.review_status='pending';

select public.refresh_base44_sync_health();
