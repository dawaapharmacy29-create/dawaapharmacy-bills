...
begin
  ...
  create temp table if not exists _smart_budget_plan(
    ...
    priority_score numeric,customer_requests_count integer,shortage_count integer,
    sales_30 numeric,sales_60 numeric,sales_90 numeric,avg_daily_usage numeric,current_stock numeric,safety_stock numeric,
    coverage_days numeric,pending_incoming numeric,demand_score numeric,
    learned_multiplier numeric,learning_confidence numeric,learning_observations integer,last_learning_outcome text,
  ...
  insert into _smart_budget_plan(
    id,product_code,product_name,original_qty,proposed_qty,unit_cost,priority_score,customer_requests_count,shortage_count,
    sales_30,sales_60,sales_90,avg_daily_usage,current_stock,safety_stock,coverage_days,pending_incoming,demand_score,
    learned_multiplier,learning_confidence,learning_observations,last_learning_outcome,
  ...
  select i.id,i.product_code,i.product_name,q.qty,q.qty,greatest(0,coalesce(i.expected_unit_cost,0)),
    coalesce(i.priority_score,0)+(live.open_count*40)+(live.shortage_count*30)+signals.demand_score
      + case when learning.learning_confidence>=0.4 then round((learning.learned_multiplier-1)*80,2) else 0 end,
    greatest(coalesce(i.customer_requests_count,0),live.open_count),live.shortage_count,
    signals.sales_30,signals.sales_60,signals.sales_90,signals.avg_daily_usage,signals.current_stock,signals.safety_stock,
    signals.coverage_days,signals.pending_incoming,signals.demand_score,
    learning.learned_multiplier,learning.learning_confidence,learning.learning_observations,learning.last_learning_outcome,
    least(q.qty,case
      ...
      when learning.learning_confidence>=0.4 and learning.learned_multiplier>=1.15 then ceil(q.qty*least(.85,.58*learning.learned_multiplier))
      when signals.demand_score>=55 then ceil(q.qty*.70)
      when signals.demand_score>=35 then ceil(q.qty*.55)
      when signals.demand_score>=20 or coalesce(i.priority_score,0)>=80 then ceil(q.qty*.45)
      when learning.learning_confidence>=0.4 and learning.learned_multiplier<=0.85 then greatest(1,ceil(q.qty*.15))
    ...
    least(q.qty,case
      ...
      when learning.learning_confidence>=0.55 and learning.learned_multiplier>=1.20 and q.qty>0 then least(q.qty,greatest(1,ceil(q.qty*.30)))
      when signals.demand_score>=45 and q.qty>0 then least(q.qty,greatest(1,ceil(q.qty*.25)))
      when q.qty>0 then 1 else 0 end),
  ...
  cross join lateral (
    select coalesce(a.sales_30,0) sales_30,coalesce(a.sales_60,0) sales_60,coalesce(a.sales_90,0) sales_90,
      coalesce(a.avg_daily_usage,0) avg_daily_usage,coalesce(a.current_stock,0) current_stock,
      ...
      greatest(-25,least(90,
        ...
        + case when coalesce(a.sales_90,0)>0 and a.sales_30>(a.sales_90/3.0)*1.25 then 18 when coalesce(a.sales_90,0)>0 and a.sales_30<(a.sales_90/3.0)*0.65 then -10 else 0 end
        + least(15,greatest(0,coalesce(a.avg_daily_usage,0)*3))
        - case when coalesce(a.pending_incoming,0)>=greatest(1,q.qty) then 12 else 0 end
      ))::numeric demand_score
    from lateral (
      select pai.* from public.purchase_analysis_items pai
      where (pai.id=i.analysis_item_id)
         or (((i.product_code is not null and pai.product_code=i.product_code) or lower(pai.product_name)=lower(i.product_name))
  ...
  cross join lateral public.smart_purchase_learning_signal(v_order.branch,i.product_code,i.product_name) learning
  where i.order_id=p_order_id;

  select coalesce(sum(proposed_total),0),count(*) filter(where customer_requests_count>0 or (learning_confidence>=0.55 and learned_multiplier>=1.20))
  ...
  while v_after>v_target+0.005 loop
    ...
    order by (customer_requests_count>0) asc,
      (learning_confidence>=0.55 and learned_multiplier>=1.20) asc,
      learned_multiplier asc,demand_score asc,priority_score asc,coverage_days desc,proposed_qty desc,unit_cost desc limit 1;
    exit when not found;
    ...
    update _smart_budget_plan set proposed_qty=proposed_qty-v_units,proposed_total=(proposed_qty-v_units)*unit_cost,
      action_reason=case
        when learning_confidence>=0.4 and learned_multiplier<=0.85 then 'تقليل كمية مبكرًا لأن نتائج الاستلامات السابقة تشير لتغطية زائدة بعد الشراء'
        when demand_score<0 then 'تقليل كمية لأن حركة الصنف أبطأ أو تغطية المخزون مرتفعة؛ الحفاظ على التنوع بدل حذف الصنف'
        when coverage_days>30 then 'تقليل كمية بسبب تغطية مخزون كافية قبل حذف أي صنف'
        else 'تقليل كمية تدريجي أولًا للحفاظ على تنوع الأصناف بدل الحذف المباشر' end
    where id=v_id;
  ...
  while v_after>v_target+0.005 loop
    ...
    order by (customer_requests_count>0) asc,(learning_confidence>=0.55 and learned_multiplier>=1.20) asc,
      learned_multiplier asc,demand_score asc,priority_score asc,coverage_days desc,proposed_qty desc limit 1;
    exit when not found;
    v_gap:=v_after-v_target; v_units:=least(v_qty-v_floor,greatest(1,ceil(v_gap/v_cost)));
    update _smart_budget_plan set proposed_qty=proposed_qty-v_units,proposed_total=(proposed_qty-v_units)*unit_cost,
      action_reason=case
        when customer_requests_count>0 then 'تقليل محسوب مع إبقاء حد يغطي طلبات العملاء والنواقص المفتوحة'
        when learning_confidence>=0.55 and learned_multiplier>=1.20 then 'تقليل محسوب مع حماية إضافية لأن النتائج السابقة تشير لخطر نقص بعد الاستلام'
        when demand_score>=45 then 'تقليل محسوب مع إبقاء مخزون أمان لصنف سريع الحركة أو منخفض التغطية'
        else 'تقليل إضافي للحد الأدنى قبل التفكير في حذف الصنف' end
    where id=v_id;
    v_after:=v_after-(v_units*v_cost);
  end loop;
  ...
  while v_after>v_target+0.005 loop
    ...
    where proposed_qty>0 and customer_requests_count=0 and demand_score<45 and unit_cost>0
      and not (learning_confidence>=0.55 and learned_multiplier>=1.10)
    order by learned_multiplier asc,demand_score asc,priority_score asc,coverage_days desc,proposed_total desc limit 1;
    exit when not found;
    update _smart_budget_plan set proposed_qty=0,proposed_total=0,
      action_reason='حذف كحل أخير بعد استنفاد تقليل الكميات؛ الصنف غير مرتبط بطلب عميل ولا توجد إشارة تعلم قوية لحمايته'
    where id=v_id;
  ...
  select count(*) filter(where proposed_qty<original_qty and proposed_qty>0),count(*) filter(where original_qty>0 and proposed_qty=0)
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'priority_score',priority_score,'customer_requests_count',customer_requests_count,'shortage_count',shortage_count,
    'sales_30',sales_30,'sales_60',sales_60,'sales_90',sales_90,'avg_daily_usage',avg_daily_usage,'current_stock',current_stock,'safety_stock',safety_stock,
    'coverage_days',coverage_days,'pending_incoming',pending_incoming,'demand_score',demand_score,
    'learned_multiplier',learned_multiplier,'learning_confidence',learning_confidence,'learning_observations',learning_observations,'last_learning_outcome',last_learning_outcome,
  ...
  ) order by (customer_requests_count>0) desc,(learning_confidence>=0.55 and learned_multiplier>=1.10) desc,demand_score desc,priority_score desc,product_name),'[]'::jsonb)
  ...
  if p_apply then
    ...
    update public.smart_purchase_order_items i set approved_quantity=p.proposed_qty,expected_total=p.proposed_total,
      customer_requests_count=p.customer_requests_count,priority_score=p.priority_score,
      supplier_reason=case when p.proposed_qty<coalesce(i.approved_quantity,i.requested_quantity,0) then concat_ws(' • ',nullif(i.supplier_reason,''),p.action_reason) else i.supplier_reason end,
      updated_at=now()
  ...
  return jsonb_build_object('ok',true,'data',jsonb_build_object(
    'target_budget',v_target,'before_total',v_before,'after_total',v_after,'savings',greatest(0,v_before-v_after),'gap_to_target',greatest(0,v_after-v_target),
    'reduced_items',v_reduced,'removed_items',v_removed,'protected_items',v_protected,'feasible',v_feasible,'applied',p_apply,
    'intelligence_version','learning_v5','plan',v_plan));
end;
$function$;
